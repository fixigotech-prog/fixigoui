...
export default function AddOfferPage() {
  ...
  return (
    <div className="container mx-auto max-w-4xl rounded-lg bg-white p-8 shadow-md">
      <h1 className="mb-6 text-2xl font-bold text-gray-900">Add New Offer</h1>
      <form onSubmit={handleSubmit} className="space-y-6">
            ...
            >
              {({ open, isLoading }) => (
                <button type="button" onClick={open} disabled={isLoading} className="w-full rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 shadow-sm hover:bg-gray-50">
                  {isLoading ? 'Loading...' : 'Upload Offer Image'}
                </button>
              )}
            </CloudinaryUploadWidget>
          </div>
        ...
        <div>
          <label htmlFor="link" className="block text-sm font-medium text-gray-700">Destination URL</label>
          <input type="url" name="link" id="link" value={link} onChange={(e) => setLink(e.target.value)} className="mt-1 block w-full rounded-md border border-gray-200 focus:border-gray-400 focus:shadow-md focus:outline-none sm:text-sm" />
        </div>

        <div>
          <label htmlFor="promocodeId" className="block text-sm font-medium text-gray-700">Promocode</label>
          <select id="promocodeId" value={promocodeId} onChange={(e) => setPromocodeId(e.target.value)} className="mt-1 block w-full rounded-md border border-gray-200 focus:border-gray-400 focus:shadow-md focus:outline-none sm:text-sm" required>
        ...
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
          <div>
            ...
            <input type="date" id="startDate" value={startDate} onChange={(e) => setStartDate(e.target.value)} className="mt-1 block w-full rounded-md border border-gray-200 focus:border-gray-400 focus:shadow-md focus:outline-none sm:text-sm" required />
          </div>
          <div>
            <label htmlFor="endDate" className="block text-sm font-medium text-gray-700">End Date</label>
            <input type="date" id="endDate" value={endDate} onChange={(e) => setEndDate(e.target.value)} className="mt-1 block w-full rounded-md border border-gray-200 focus:border-gray-400 focus:shadow-md focus:outline-none sm:text-sm" required />
          </div>
        </div>
